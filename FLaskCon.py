...
def Added_Comment_Annalysic():
	...
	DB_conn.close()
		...
		#还要修改RS中的字典
	Dict_Pre_Score = RS.Food_AveScore[Comment_Food]['AveScore'] * RS.Food_AveScore[Comment_Food]['Times'] 
	RS.Food_AveScore[Comment_Food]['Times'] += 1
	RS.Food_AveScore['AveScore'] = (Dict_Pre_Score + Score) / RS.Food_AveScore[Comment_Food]['Times'] 
...
if __name__ == '__main__':
	...
	while (True):
		RS.Cal_ItemCF()
		RS.Cal_UserCF()
		app.run(debug = False, threaded = True)	#linux下要尝试用gunicorn
		time.sleep(seconds)
		RS.Cal_ItemCF()
		RS.Cal_UserCF()
	#app.run(debug = False, threaded = True)	#linux下要尝试用gunicorn