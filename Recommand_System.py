...
class RSystem:
	def __init__(self):
		self.U_T = dict()	#用户标签字典	{"user1": {"tag1": 1/0, "tag2" : 1/0} ,"user2": {"tag1": 1/0, "tag2" : 1/0} }
		self.F_T = dict()	#食物标签字典
		self.Food_ID_List  = ['947232']	#在使用前在此处初始化
		self.User_ID_List = ['13342','34221']	
		...
		DB_conn =  pymysql.connect(host = 'localhost',user = 'mozart',password = 'mozewei19980206',db = 'db', charset = 'utf8mb4',cursorclass = pymysql.cursors.DictCursor)
		Get_F_AveS = DB_conn.cursor()
		Get_F_AveS.execute("select * from F_AveS")
		rows = Get_F_AveS.fetchall()

		for row in rows:
			self.Food_AveScore[row['Food_ID']] = row['Ave_Score']
		Get_F_AveS.close()
		DB_conn.close()
	...
	def DOWN_Tags_Weight(self,user_id,Dislike_Food):
		...
		for down_tag_index in Unlike_tag_set:
			...
			Down_User_Tags_Weight.execute(' update UTs set %s *= 0.8 where User_ID = %s' %(tag,user_id))
	...
	def Renew_Food_Rank_List(self):
		if(isinstance(self.Food_AveScore,dict)):
			temp = sorted(self.Food_AveScore.items(),key = lambda item : item[0], reverse = True)
			print(temp)
			self.Food_AveScore = temp
		elif(isinstance(self.Food_AveScore,list)):	
			temp = sorted(self.Food_AveScore,key = lambda item :item[1],reverse = True )
			print(temp)
			self.Food_AveScore = temp
	...
	def Recommand(self,user_id):    #根据用户ID返回其推荐列表
		...
		for key,value in self.Food_AveScore:
